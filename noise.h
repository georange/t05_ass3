	...
	for (int o = 0; o < octaves; o++) {
		for (int i = 0; i < width; ++ i) {
			for (int j = 0; j < height; ++ j) {
				...
				float atl = a.dot(topleft);
				float btr = b.dot(topright);
				float cbr = c.dot(bottomright);
				float dbl = d.dot(bottomleft);
				
				float x1 = mix(atl, dbl, f(dy));
				float x2 = mix(btr, cbr, f(dy)); 

				float noise = mix(x1, x2, f(dx));;

				noise_data[i + j * height] += noise * amplitude * 0.55;
			}
		}
		period = period/2;
